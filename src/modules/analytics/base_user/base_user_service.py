...
async def get_link_with_clicks_by_url(
...
async def get_click_counter_for_link_by_url(
    link_url: str, repo: Annotated[LinkRepository, Depends(get_link_repository)]
) -> int:
    link = await repo.get_link_by_url(link_url)
    return int(link.click_counter)
...
async def get_list_of_distribution_by_week_days_for_user(
    user_id: int,
...
async def get_full_distribution_by_week_days_for_user(
    ...
    links_stats = await get_list_of_distribution_by_week_days_for_user(
